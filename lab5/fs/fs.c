...
int dirty_block(u_int blockno) {
	...
	return syscall_mem_map(0, (void *)va, 0, (void *)va, PTE_D | PTE_DIRTY);
}
...
int read_block(u_int blockno, void **blk, u_int *isnew) {
	...
	if (block_is_mapped(blockno)) { // the block is in memory
		...
		syscall_mem_alloc(0, (void *)va, PTE_D);
...
void check_write_block(void) {
	...
	// clear it out
	syscall_mem_unmap(0, (void *)diskaddr(1));
	user_assert(!block_is_mapped(1));
...
//
// Post-Condition:
//	Return 0: success, and set the pointer to the target block in *ppdiskbno(Note that the
// pointer 			might be NULL). 		-E_NOT_FOUND if the function needed
// to allocate
// an indirect block, but alloc was 0. 		-E_NO_DISK if there's no space on the disk for an
// indirect block. 		-E_NO_MEM if there's no space in memory for an indirect block.
